...
function checkEmail(email){
    return new Promise((resolve, reject) => {
        var sql = "SELECT * FROM `bilsportdb`.`t_badminton` WHERE `email` = ?;";
...
function checkId(bilkentId){
    return new Promise((resolve, reject) => {
        var sql = "SELECT * FROM `bilsportdb`.`t_badminton` WHERE `bilkentId` = ?;";
...
router.get('/', (req, res) => {
    mysqlConnection.query('SELECT * FROM t_badminton', (err, rows, fields) => {
        if (!err)
            res.send(rows);
        else
            console.log(err);
...
router.get('/:id', (req, res) => {
    mysqlConnection.query('SELECT * FROM t_badminton WHERE id = ?', [req.params.id], (err, rows, fields) => {
        if (!err)
            res.send(rows);
        else
            console.log(err);
...
router.delete('/:id', (req, res) => {
    mysqlConnection.query('DELETE FROM t_badminton WHERE id = ?', [req.params.id], (err, rows, fields) => {
        if (!err)
...
router.post('/', (req, res) => {
    ...
    emailCheck.then((value) => {
        idCheck.then((value) => {
            var sql = "INSERT INTO `bilsportdb`.`t_badminton`(`bilkentId`, `email`, `ge`) VALUES  (?,?,?);";
